"""Config flow to configure the Nokia integration."""
...
from .const_nokia import DEFAULT_HOST, DEFAULT_USER 
...
class NokiaFlowHandler(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow."""

    VERSION = 1

    def __init__(self) -> None:
        """Initialize the Nokia config flow."""
        self.placeholders = {
    ...
    async def async_step_user(self, user_input=None):
        ...
        if info.get("Vendor") is not None and info.get("ModelName") is not None:
            name = f"{info['Vendor']} - {info['ModelName']}"
        else:
